...
func TestSerializeWithPassword(t *testing.T) {
	serialized, err := SerializeWithPassword([]byte("hello world"), []byte("supersecret"))
	assert.Nil(t, err)

	deserialized, err := DeserializeWithPassword(serialized, []byte("supersecret"))
	assert.Nil(t, err)
